...
fn create_ca() -> Result<CertPem> {
    let (cert, key) = generate_ca(
        "CN", "hkjc", "Acme Inc.", CertSigAlgo::EcDsa, None, Some(10*365)
    )?;
...
fn create_cert(ca:&certify::CA, domains: Vec<&str>, cn: &str, is_client: bool ) -> Result<CertPem>{
    ...
    let (cert, key) = generate_cert(
        ca, domains, "CN", "hkjc", cn, CertSigAlgo::EcDsa, None, is_client, days
    )?;
